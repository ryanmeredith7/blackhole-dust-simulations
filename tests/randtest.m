...
animate(xs, a, speed, file="plots/testRA");
...
animate(xs, b, speed, file="plots/testRB");
%uiwait(msgbox("Press OK to continue.", "Done plotting beta"));

%figure(Name="Animation of rho");
animate(xs(2:end) - dx/2, diff(xs .^ 3 .* (sin(b) .^ 2 + a)) ./ (8*pi*dx .* (xs(2:end) - dx/2) .^ 2), speed, [0, 0.3], file="plots/testRP");
